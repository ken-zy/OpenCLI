...
## Task 2: 更新 5 份 SKILL.md blockquote
...
四份（browser/explorer/oneshot/autofix）blockquote 内容完全一致；smart-search 多一行特例。
...
git commit -m "docs(skills): sync 5 SKILL.md blockquotes to harness-hook-aware rules
...
## Task 3: 更新项目 `.claude/CLAUDE.md` + `AGENTS.md`

**Files:**
- Modify: `.claude/CLAUDE.md`（line 63 + 新增 harness hook 小节）
- Modify: `AGENTS.md`（line 63 typo + 同步 CLAUDE.md 的 harness hook 小节）

`.claude/CLAUDE.md` 已 git tracked；`AGENTS.md` 本地私有（在 `.git/info/exclude` 里）。

- [ ] **Step 1: 修改 `.claude/CLAUDE.md` 的"唯一正本预检脚本"行**

Edit `.claude/CLAUDE.md:63`，把：
...
- [ ] **Step 2: 在 `.claude/CLAUDE.md` 现有"0号 Chrome 绑定"小节之后追加新小节**

在第一处"### upstream rebase 时的处理"之前（约 line 74 位置）插入：

```markdown
### Harness Hook 强制层（2026-04-20）

除 5 份 SKILL.md 的 blockquote 文档层外，另有 harness 层强制兜底：
...
- `~/.claude/settings.json` 的 `hooks.PreToolUse[Bash]` 绑定

即便 AI 漏读 skill blockquote，Bash 执行前 hook 会自动拦截非 bypass 的 opencli 命令并跑预检。
```

- [ ] **Step 3: 修改 `AGENTS.md` 同步 CLAUDE.md 变更**

AGENTS.md 是 CLAUDE.md 的本地私有副本（line 63 有 typo `.Codex` 实应 `.claude`）。把 AGENTS.md 的 "## jdy Main 项目私有补丁：0号 Chrome 绑定（2026-04-18）" 小节整体替换为 CLAUDE.md 对应小节的最新内容（包括 Step 1 的路径修复 + Step 2 新增的 harness hook 小节）。
...
Expected: CLAUDE.md grep 命中新路径；AGENTS.md 不再有 `.Codex`；两文件各有 1 处"Harness Hook 强制层"。

- [ ] **Step 5: Commit（仅 CLAUDE.md；AGENTS.md 在 exclude 中不入 git）**
...
# [1] Self-check — doesn't depend on command
self_check_failed=""
[ -x "$PREFLIGHT_SCRIPT" ] || self_check_failed="preflight not executable: $PREFLIGHT_SCRIPT"
[ -z "$self_check_failed" ] && ! command -v opencli >/dev/null 2>&1 && \
  self_check_failed="opencli not in PATH"
[ -z "$self_check_failed" ] && [ ! -r "$BYPASS_FILE" ] && \
  self_check_failed="bypass list unreadable: $BYPASS_FILE"
...
# Past this point, the command appears to involve opencli. Self-check must pass.
if [ -n "$self_check_failed" ]; then
  cat >&2 <<EOF
[opencli-preflight-guard] FAIL-CLOSED: $self_check_failed
命令可能涉及 opencli 但 guard 基础设施未就绪。修复建议：
  - 检查 $PREFLIGHT_SCRIPT 是否存在且 +x
  - 检查 opencli 是否在 PATH（which opencli）
  - 检查 $BYPASS_FILE 是否存在（跑 ~/.claude/scripts/gen-bypass-list.sh）
EOF
  exit 2
fi
...
import json, shlex, os, subprocess, sys, pathlib
...
def scan(cmd_str, depth=0):
    """Return 'bypass' / 'need_preflight' / 'fail_closed:<reason>' for a command string."""
    try:
        tokens = shlex.split(cmd_str, posix=True)
    except ValueError as e:
        return f"fail_closed:shlex: {e}"

    # [3] Wrapper detection + recursion
    ...
    # [5] Find independent opencli tokens
    opencli_positions = []
    for i, tok in enumerate(tokens):
        if "=" in tok and i == 0:
            continue  # env var prefix FOO=bar
        if os.path.basename(tok) == "opencli":
            opencli_positions.append(i)

    if not opencli_positions:
        # No independent opencli token; grep matched a substring (e.g., opencli-autofix)
        return "bypass"

    # Evaluate each opencli occurrence
    for pos in opencli_positions:
        next1 = tokens[pos + 1] if pos + 1 < len(tokens) else ""
        next2 = tokens[pos + 2] if pos + 2 < len(tokens) else ""

        if next1 in TOP_BYPASS:
            continue  # this occurrence is bypass
        if next1 in TOP_NEED_PREFLIGHT:
            return "need_preflight"

        # <site>/<cmd> lookup in bypass list
        if next1 and next2:
            site_cmd = f"{next1}/{next2}"
            try:
                with open(bypass_file, "r") as f:
                    bypass_set = {line.strip() for line in f
                                  if line.strip() and not line.startswith("#")}
                if site_cmd in bypass_set:
                    continue
            except Exception:
                return f"fail_closed:bypass list read: {bypass_file}"

        return "need_preflight"

    return "bypass"  # all occurrences bypassed
...
在本步标记：某些 case（`opencli hackernews top`、`opencli v2ex hot`、`opencli 36kr hot` 等命令级匹配）依赖 Task 5 生成的 bypass_commands.txt。当前先跑 self-check / 顶层 bypass / wrapper / fail-closed 相关 case。
...
```bash
cd /Users/jdy/Documents/Main
git commit -m "refactor: migrate opencli-specific docs+script to opencli repo
